...
static void align_rep(ThreadPool& tp, const ChunkSeqs& chunk_seqs, vector<PairEntryShort>::const_iterator begin, vector<PairEntryShort>::const_iterator end, int64_t max_oid, BufferArray& out, ofstream* aln_out, ClusterStats &cstats) {
	...
	for (const Hsp& h : hsps) {
		...
		if (aln_out) {
			*aln_out << rep_oid << '\t' << member_oid << '\t' << h.query_cover_percent(rep.length()) << '\t' << h.subject_cover_percent(member.length()) << '\t' << h.evalue << endl;
		}
		...
		if (unid) {
			...
			if (h.subject_cover_percent(member.length()) >= config.member_cover.get(80)) {
				const Edge e(rep_oid, member_oid, rep.length(), member.length());
				out.write_msb(e);
				pass = true;
			}
			if (h.query_cover_percent(rep.length()) >= config.member_cover.get(80)) {
				Edge e(member_oid, rep_oid, member.length(), rep.length());
				out.write_msb(e);
				pass = true;
			}
			if (pass)
				++cstats.hits_filtered;
		}
			...
			Edge e(oid1, oid2, len1, len2);
			out.write_msb(e);
			++cstats.hits_filtered;
		}
	}
	cstats.hits_evalue_filtered += hsps.size();
}
...
RadixedTable align(Job& job, int chunk_count, int64_t db_size) {
	...
	mkdir(aln_path);
	unique_ptr<FileArray> output_files(new FileArray(aln_path, RADIX_COUNT, job.worker_id(), false));
	Atomic queue(queue_path, job);
	int chunk, chunks_processed = 0;
	ClusterStats cstats_all;
	while (true) {
		...
		std::function<void()> align_worker = [&]() {
			...
			unique_ptr<ofstream> aln_out;
			ClusterStats cstats;
			if (!config.aln_out.empty()) {
				std::ostringstream ss;
				ss << config.aln_out << '.' << std::this_thread::get_id();
				aln_out.reset(new ofstream(ss.str(), std::ios_base::app | std::ios_base::out));
				if (!aln_out->good())
					throw std::runtime_error("Failed to open alignment output file: " + ss.str());
			}
			size_t size;
			...
			while (it.good()) {
				align_rep(tp, *chunk_seqs, it.begin(), it.end(), db_size, edge_buf, aln_out.get(), cstats);
				++it;
			}
			cstats_all.add(cstats);
			};
		task_set.enqueue(align_worker);
	...
	Atomic finished(aln_path + PATH_SEPARATOR + "finished", job);

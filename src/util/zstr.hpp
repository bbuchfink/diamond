        ...
        if (this->gptr() == this->egptr())
        {
            ...
            do
            {
                ...
                else
                {
                    ...
                    zstrm_p->next_in = reinterpret_cast<Bytef*>(in_buff_start);
                    zstrm_p->avail_in = (uInt)(in_buff_end - in_buff_start);
                    zstrm_p->next_out = reinterpret_cast<Bytef*>(out_buff_free_start);
                    ...
                    in_buff_start = reinterpret_cast<char*>(zstrm_p->next_in);
                    in_buff_end = in_buff_start + zstrm_p->avail_in;
                    out_buff_free_start = reinterpret_cast<char*>(zstrm_p->next_out);
...
public:
    ...
    int deflate_loop(int flush)
    {
        while (true)
        {
            zstrm_p->next_out = reinterpret_cast<Bytef*>(out_buff);
            ...
            std::streamsize sz = sbuf_p->sputn(out_buff, reinterpret_cast<char*>(zstrm_p->next_out) - out_buff);
            if (sz != reinterpret_cast<char*>(zstrm_p->next_out) - out_buff)
            {
	...
	virtual ~ostreambuf()
    {
        ...
        if (sync() != 0) std::cerr << "~ostreambuf(): error in sink stream";
        delete [] in_buff;
        delete [] out_buff;
        delete zstrm_p;
    }
    ...
    {
        zstrm_p->next_in = reinterpret_cast<Bytef*>(pbase());

...
File& operator>>(File& file, ReferenceHeader& h)
{
	...
	file.read(h.build);
	h.build = big_endian_byteswap(h.build);
	file.read(h.db_version);
	h.db_version = big_endian_byteswap(h.db_version);
	file.read(h.sequences);
	h.sequences = big_endian_byteswap(h.sequences);
	file.read(h.letters);
...
File& operator>>(File& file, SequenceFile::SeqInfo& r) {
	uint32_t p;
	file.read(r.pos);
	r.pos = big_endian_byteswap(r.pos);
	file.read(r.seq_len);
	r.seq_len = big_endian_byteswap(r.seq_len);
	file.read(p);
	return file;
}
...
DatabaseFile::DatabaseFile(const string& input_file, Flags flags, const ValueTraits& value_traits) :
	...
	file_(auto_append_extension_if_exists(input_file, FILE_EXTENSION), "rb")
{
	init(flags);
	...
	if (flag_any(flags, Flags::TAXON_MAPPING)) {
		file_.seek(header2.taxon_array_offset);
		taxon_list_.reset(new TaxonList(file_, ref_header.sequences, header2.taxon_array_size));
	}

	if (flag_any(flags, Flags::TAXON_SCIENTIFIC_NAMES)) {
		file_.seek(header2.taxon_names_offset);
		deserialize(file_, taxon_scientific_names_);
	}

	if (flag_any(flags, Flags::TAXON_NODES)) {
		file_.seek(header2.taxon_nodes_offset);
		taxon_nodes_.reset(new TaxonomyNodes(file_, ref_header.build));
		init_cache();
	}
...
bool DatabaseFile::eof() const {
	return tell_seq() == sequence_count().value();
}

void DatabaseFile::init_seq_access() {
	file_.seek(sizeof(ReferenceHeader) + sizeof(ReferenceHeader2) + 8);
}
...
bool DatabaseFile::read_seq(vector<Letter>& seq, string &id, std::vector<char>* quals)
{
	...
	seq.clear();
	id.clear();
	file_.read_to(std::back_inserter(seq), '\xff');
	file_.read_to(std::back_inserter(id), '\0');
	return false;
}
...
optional<uint64_t> DatabaseFile::sequence_count() const {
	return ref_header.sequences;
}

optional<uint64_t> DatabaseFile::letters() const {
...
int DatabaseFile::rank(TaxId taxid) const
{
	const int r = taxon_nodes_->rank(taxid);
	return ref_header.build <= 165 && r >= 0 ? Rank::legacy_rank_codes.at(r) : r;
}
...
void DatabaseFile::read_seqid_list() {
	...
	if (flag_any(flags_, Flags::NEED_LENGTH_LOOKUP))
		seq_length_.reserve(sequence_count().value());
	OId oid = 0;
	vector<Letter> seq;
	string id;
	init_seq_access();
...
TaxId DatabaseFile::max_taxid() const {
	return taxon_nodes_->max();
}

TaxId DatabaseFile::get_parent(TaxId taxid) {
	return taxon_nodes_->get_parent(taxid);
}

void DatabaseFile::print_info() const {
	*message_stream << "Database: " << config.database << ' ';
	*message_stream << "(type: " << to_string(type()) << ", ";
	*message_stream << "sequences: " << sequence_count().value() << ", ";
	*message_stream << "letters: " << letters().value() << ", build version: " << ref_header.build << ')' << endl;
}
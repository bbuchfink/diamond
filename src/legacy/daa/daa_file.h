...
{
	DAA_header2():
		diamond_build(config.daa_build_version == 0 ? Const::build_version : config.daa_build_version)
	{
...
struct DAAFile
{
		...
		query_count_ (0)
	{
		...
		f_.read(ref_len_.data(), sizeof(uint32_t) * (size_t)h2_.db_seqs_used);
